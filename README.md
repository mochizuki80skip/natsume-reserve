...
| `/admin/login` | 本部・店舗 | 店舗コード＋パスワードでログイン |
| `/admin/login/<店舗コード>` | 店舗 | 店舗専用のログインURL。店舗名が表示され、パスワードだけでログイン |
...
| `/admin/reservations` | 店舗 | 予約・来院ログ（WEB予約一覧／キャンセル名簿）。期間・氏名で絞り込み、CSV／Excel 書き出し |
| `/admin/settings` | 店舗 | 店舗名・電話・ベッド数・公開日数・営業時間の個別設定・パスワード変更 |
| `/admin/story` | 店舗 | Instagram ストーリーズ用の「本日の空き状況」画像（共有／保存） |
| `/admin/print/<日付>` | 店舗 | A4 印刷／PDF 保存用の予約表 |
| `/admin/hq` | 本部 | 店舗の追加・停止・パスワード再設定、全店共通設定（営業時間・締切・電話マーク閾値など） |
| `/admin/hq/overview` | 本部 | 全店状況：指定日の全店舗の予約・来院・WEB・初診／再来／自賠・キャンセルの一覧と合計 |

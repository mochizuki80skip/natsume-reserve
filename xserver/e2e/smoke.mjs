// Xserver（PHP）版：起動中のサーバー（BASE_URL）に対して顧客予約〜管理画面までを一通り確認する
// 例：cd xserver && php -S 127.0.0.1:3003 -t public dev-router.php &  BASE_URL=http://127.0.0.1:3003 HQ_PASSWORD=... node e2e/smoke.mjs
import { createRequire } from 'node:module';
const { chromium } = createRequire(new URL('../frontend/package.json', import.meta.url))('playwright'); // frontend の devDependencies を使う
...
// テスト用の日付：3 日後以降で〇のある営業日を週間 API から選ぶ（木・日・祝日を避ける）
...
console.log('test dates:', D2, D3, D4);
...
await ap.waitForURL(/\/admin\/login/, { timeout: 15000 });
...
// 店舗専用ログインURL（パスワードのみ）
{
  ...
  await spp.goto(`${BASE}/admin/login/NOPE`);
  await spp.waitForSelector('text=ページが見つかりません');
  ok('存在しない店舗コードは「見つかりません」', true);
  await sp.close();
}
...
await hp.waitForSelector('h1:has-text("本部管理")');

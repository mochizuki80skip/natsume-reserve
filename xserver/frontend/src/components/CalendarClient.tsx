...
interface Props { month: string; today: string; days: Day[]; publishDaysAhead: number; onRefresh: () => void }
...
export default function CalendarClient({ month, today, days, publishDaysAhead, onRefresh }: Props) {
  const [state, setState] = useState(days);
  useEffect(() => setState(days), [days]);
  ...
  return (
    <div>
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <Link to={`/admin/calendar?month=${shiftMonth(month, -1)}`} className="rounded border bg-white px-3 py-1">‹ 前月</Link>
        <h1 className="text-xl font-bold">{Number(month.slice(0, 4))}年{Number(month.slice(5))}月</h1>
        <Link to={`/admin/calendar?month=${shiftMonth(month, 1)}`} className="rounded border bg-white px-3 py-1">翌月 ›</Link>
      </div>
        ...
        {state.map((d) => {
          ...
          return (
            <button key={d.date} type="button" disabled={past} onClick={() => apply(d)}
              ...
              <Link to={`/admin/day/${d.date}`} className="mt-1 text-[10px] text-brand underline" onClick={(e) => e.stopPropagation()}>予約表</Link>
            </button>
          );
        })}
      </div>
